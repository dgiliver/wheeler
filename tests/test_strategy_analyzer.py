...
@pytest.fixture
def analyzer():
    return StrategyAnalyzer(max_spread_percentage=15.0, min_volume=50, min_open_interest=100)


@pytest.fixture
def liquid_option():
    return OptionQuote(
        bid=Decimal("1.00"), ask=Decimal("1.10"), volume=200, open_interest=500, implied_volatility=30.0, delta=0.3
    )
...
def test_csp_entry_conditions(analyzer, liquid_option):
    ...
    price_data = pd.DataFrame({"close": [100, 95, 90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30]})
...
def test_assignment_decision():
    ...
    should_accept, reason = analyzer.accept_assignment(
        cost_basis=Decimal("100"), current_price=Decimal("95"), implied_volatility=60
    )
...
def test_should_force_exit_on_wide_spread(analyzer, illiquid_option):
    ...
    should_exit, reason = analyzer.should_force_exit(
        position=position, current_price=Decimal("2.80"), option_quote=illiquid_option
    )
    assert should_exit
...
def test_csp_entry_illiquid_option(analyzer, illiquid_option):
    ...
    price_data = pd.DataFrame({"close": [100, 95, 90, 85, 80]})
...
def test_should_force_exit_no_max_loss():
    ...
    should_exit, reason = analyzer.should_force_exit(position=position, current_price=Decimal("140"))
