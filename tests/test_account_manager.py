...
def test_can_open_position_new(account_manager):
    ...
    assert account_manager.can_open_position(Decimal("19000"))
    assert not account_manager.can_open_position(Decimal("21000"))
...
def test_can_open_position_existing(account_manager):
    ...
    account_manager.positions.append(
        Position(symbol="AAPL", entry_price=Decimal("150"), quantity=100, position_type=PositionType.STOCK)
    )

    # Try to add more to AAPL position
    # Current position cost = 15000, max = 20000
    assert account_manager.can_open_position(Decimal("4000"), "AAPL")
    assert not account_manager.can_open_position(Decimal("6000"), "AAPL")
...
def test_get_max_contracts_existing_position(account_manager):
    ...
    max_contracts = account_manager.get_max_contracts(strike_price=Decimal("150"), existing_symbol="AAPL")
...
def test_get_max_contracts_different_symbol(account_manager):
    ...
    max_contracts = account_manager.get_max_contracts(strike_price=Decimal("100"), existing_symbol="AMD")
...
def test_day_trade_tracking():
    ...
    account.update_pdt_status(equity=Decimal("20000"), daytrade_count=2, is_pattern_day_trader=False)

    # 2 day trades used, 1 remaining - should be allowed
    can_trade, _ = account.can_day_trade()
    assert can_trade
    assert not account.would_exceed_day_trades()

    # Update to 3 day trades - should be blocked
    account.update_pdt_status(equity=Decimal("20000"), daytrade_count=3, is_pattern_day_trader=False)

    can_trade, reason = account.can_day_trade()
    assert not can_trade
    assert "PDT limit reached" in reason
    assert account.would_exceed_day_trades()
...
def test_pdt_not_restricted_above_threshold():
    ...
    account.update_pdt_status(equity=Decimal("50000"), daytrade_count=3, is_pattern_day_trader=False)
