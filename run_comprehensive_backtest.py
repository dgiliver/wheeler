...
from src.backtesting.wheel_backtester import WheelBacktester  # noqa: E402
from src.config.settings import AlpacaConfig, StockConfig  # noqa: E402
...
def create_config(delta_min: float, delta_max: float) -> AlpacaConfig:
    ...
    watchlist = [
        StockConfig(symbol=symbol, max_position_size=max_pos, min_strike_delta=delta_min, max_strike_delta=delta_max)
...
def run_single_backtest(
    period_name: str, start_date: str, end_date: str, delta_name: str, delta_min: float, delta_max: float, rsi: int
) -> BacktestResult:
    ...
    try:
        backtester = WheelBacktester(
            config=config, start_date=start, end_date=end, initial_capital=INITIAL_CAPITAL, rsi_oversold=rsi
        )
...
def run_all_backtests() -> list[BacktestResult]:
    ...
    print(f"\n{'=' * 70}")
    print("COMPREHENSIVE WHEEL STRATEGY BACKTEST")
    print(f"{'=' * 70}")
    ...
    for period_name, start, end in BACKTEST_PERIODS:
        ...
        for delta_name, delta_min, delta_max in DELTA_CONFIGS:
            for rsi in RSI_THRESHOLDS:
                ...
                result = run_single_backtest(period_name, start, end, delta_name, delta_min, delta_max, rsi)
...
def analyze_results(results: list[BacktestResult]):
    ...
    valid_sharpe = (
        ...
        .nlargest(10, "sharpe_ratio")[
            ["period", "delta_config", "rsi_threshold", "sharpe_ratio", "total_return", "max_drawdown", "win_rate"]
        ]
        .copy()
    )
    ...
    best_conservative = (
        df[df["max_drawdown"] > -0.15].nlargest(1, "total_return").iloc[0]
        if len(df[df["max_drawdown"] > -0.15]) > 0
        else None
    )
    ...
    if best_conservative is not None:
        print()
        print(
            f"   • Best Conservative (<15% drawdown): {best_conservative['delta_config']} delta, RSI={best_conservative['rsi_threshold']}"
        )
        print(
            f"     → {best_conservative['total_return']:.1%} return, {best_conservative['max_drawdown']:.1%} max drawdown"
        )
