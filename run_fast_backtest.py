...
class PriceCache:
    ...
    def get_price(self, symbol: str, date: datetime) -> float | None:
    ...
    def get_rsi(self, symbol: str, date: datetime, period: int = 14) -> float | None:
...
def find_put_by_delta(price: float, dte: int, min_delta: float, max_delta: float, iv: float = 0.35) -> dict | None:
    ...
    for pct in range(-30, -4, 1):
        ...
        if min_delta <= delta <= max_delta:
            ...
            return {"strike": strike, "premium": premium, "delta": delta, "dte": dte}
    return None


def find_call_by_delta(price: float, cost_basis: float, dte: int, iv: float = 0.35) -> dict | None:
...
class FastBacktester:
    def __init__(self, cache: PriceCache, capital: float, min_delta: float, max_delta: float, rsi_threshold: int):
        self.cache = cache
    ...
    def _handle_cc_expiration(self, date: datetime, pos: Position):
        ...
        if price > pos.strike:
            ...
            stock_pos = next((p for p in self.positions if p.symbol == pos.symbol and p.position_type == "STOCK"), None)
            if stock_pos:
    ...
    def _generate_results(self, start: datetime, end: datetime) -> BacktestResult:
        ...
        total_closed = len(
            [t for t in self.trades if t["action"] in ["EXPIRED", "CC_EXPIRED", "CALLED_AWAY", "ASSIGNED"]]
        )
    ...
    for period_name, start_str, end_str in BACKTEST_PERIODS:
        ...
        for delta_name, min_d, max_d in DELTA_CONFIGS:
            for rsi in RSI_THRESHOLDS:
                ...
                print(
                    f"[{current}/{total_tests}] {period_name} | {delta_name} | RSI={rsi} → "
...
def analyze_results(results: list[BacktestResult]):
    ...
    for _, row in top.iterrows():
        print(
            f"  {row['period']:10} | {row['delta_config']:18} | RSI={row['rsi_threshold']:2} | "
    ...
    for name, row in delta_avg.iterrows():
        print(
            f"  {name:18} | Return: {row['total_return']:+5.1%} | "
    ...
    rsi_avg = df.groupby("rsi_threshold").agg(
        {"total_return": "mean", "sharpe_ratio": "mean", "total_trades": "mean", "premium_collected": "mean"}
    )
    for rsi, row in rsi_avg.iterrows():
        print(
            f"  RSI={rsi:2} | Return: {row['total_return']:+5.1%} | "
    ...
    period_avg = df.groupby("period").agg(
        {"total_return": "mean", "sharpe_ratio": "mean", "max_drawdown": "mean", "assignments": "mean"}
    )
    for period, row in period_avg.iterrows():
        print(
            f"  {period:10} | Return: {row['total_return']:+5.1%} | "
            f"Sharpe: {row['sharpe_ratio']:5.2f} | DD: {row['max_drawdown']:5.1%} | "
...
"""
    ...
    # RSI insight
    df.groupby("rsi_threshold")["total_return"].mean()
    if best_rsi >= 40:
