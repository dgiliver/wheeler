...
class StrategyAnalyzer:
    def __init__(self, 
                 ...
                 take_profit_percentage: float = 60.0): # Take profit at 60% max profit
    ...
    def should_enter_csp(self, 
                        price_data: pd.DataFrame,
                        option_quote: OptionQuote) -> Tuple[bool, str]:
        """
    ...
    def should_force_exit(self,
                         ...
                         option_quote: Optional[OptionQuote] = None) -> Tuple[bool, str]:
    ...
    def should_roll_option(self, days_to_expiry: int, current_strike: Decimal, 
                          current_price: Decimal, is_put: bool, option_quote: OptionQuote) -> tuple[bool, str]:
        """
    ...
    def get_optimal_exit_price(self, 
    ...
    def accept_assignment(self, 
                         cost_basis: Decimal,
                         current_price: Decimal,
                         implied_volatility: float) -> Tuple[bool, str]:
    ...
    def analyze_put_opportunity(self, 
                              symbol: str, 
                              price_data: pd.DataFrame, 
                              options_chain: List[Dict],
                              min_delta: float = 0.20,
                              max_delta: float = 0.45) -> Optional[Dict]:
        ...
        if best_option:
            logger.info(f"Selected put option for {symbol}: Strike={best_option['strike']}, Premium={best_option.get('bid')}")
            
        return best_option

    def analyze_call_opportunity(self, 
                               symbol: str, 
                               cost_basis: Decimal, 
                               options_chain: List[Dict],
                               min_delta: float = 0.15,
                               max_delta: float = 0.40) -> Optional[Dict]:
        ...
        if best_option:
            logger.info(f"Selected call option for {symbol}: Strike={best_option['strike']}, Premium={best_option.get('bid')}")
