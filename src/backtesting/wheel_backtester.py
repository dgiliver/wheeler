...
class WheelBacktester:
    def __init__(
        ...
        self.data_provider = AlpacaHistoricalDataProvider(api_key=config.api_key, api_secret=config.api_secret)
    ...
    def _can_enter_new_position(self, stock: StockConfig) -> bool:
        ...
        symbol_positions_value = sum(
            float(pos.quantity * pos.entry_price) for pos in self.positions if pos.symbol == stock.symbol
        )
    ...
    def _handle_option_expiration(self, date: datetime, position: Position):
        ...
        if position.position_type == PositionType.CASH_SECURED_PUT:
            if current_price < float(position.strike_price):
                ...
                logger.info(
                    f"ASSIGNED (PUT): {position.symbol} at {position.strike_price}. Capital: {self.current_capital}"
                )
            else:
                # Expiration (PUT)
                self.trades.append(
            ...
            if current_price > float(position.strike_price):
                ...
                for p in self.positions:
                    if (
                        ...
                        and p.position_type == PositionType.STOCK
                        and p.quantity >= position.quantity  # Assuming sufficient coverage
                    ):
                        stock_to_remove = p
                        break
                ...
                self.current_capital += position.strike_price * Decimal(str(position.quantity))
                logger.info(
                    f"ASSIGNED (CALL): {position.symbol} at {position.strike_price}. Stocks sold. Capital: {self.current_capital}"
                )
            else:
                # Expiration (CALL)
                self.trades.append(
    ...
    def _try_sell_covered_call(self, date: datetime, position: Position):
        ...
        existing_cc = any(
            p.symbol == position.symbol and p.position_type == PositionType.COVERED_CALL for p in self.positions
        )
        if existing_cc:
            return
        ...
        chain = self.data_provider.get_historical_options(
            position.symbol, date, self._get_next_monthly_expiration(date)
        )
    ...
    def _sell_call(self, date: datetime, call: dict, stock_position: Position):
        ...
        logger.info(
            f"SELL CC: {stock_position.symbol} Strike={call['strike']} Exp={call['expiration']} Premium={premium}"
        )
    ...
    def _extract_expiration(self, option_id: str) -> datetime:
        ...
        try:
            ...
            pass  # nosec B110 - fallback to default expiration below
    ...
    def _record_portfolio_value(self, date: datetime):
        ...
        self.daily_portfolio_value.append({"date": date, "portfolio_value": total_value})
    ...
    def _generate_per_stock_summary(self) -> dict:
        ...
        for symbol, data in summary.items():
            ...
            if shares > 0:
                ...
                final_summary[symbol] = {"status": "EXITED", "total_premium": float(premium), "net_pnl": float(net_pnl)}
    ...
    def _generate_backtest_results(self) -> dict:
        ...
        total_closed_actions = len(
            [t for t in self.trades if t["action"] in ["PUT_EXPIRED", "CALL_EXPIRED", "CALL_ASSIGNED", "PUT_ASSIGNED"]]
        )
    ...
    def _find_entry_opportunity(self, date: datetime, stock: StockConfig):
        ...
        chain = self.data_provider.get_historical_options(
            symbol=stock.symbol, date=date, expiration=self._get_next_monthly_expiration(date)
        )

        put = self.strategy_analyzer.analyze_put_opportunity(
            symbol=stock.symbol, price_data=price_data, options_chain=chain
        )
