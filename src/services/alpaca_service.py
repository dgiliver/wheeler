...
class AlpacaService:
    ...
    def get_option_chain(self, symbol: str, expiration: datetime, min_dte: int = None, max_dte: int = None) -> list:
        ...
        try:
            ...
            if min_dte is not None and max_dte is not None:
                ...
                logger.debug(
                    f"Fetching option chain for {symbol} with DTE {min_dte}-{max_dte} ({min_exp} to {max_exp})"
                )
            else:
                min_exp = target_date
                max_exp = target_date
            ...
            while pages_fetched < max_pages:
                ...
                for contract_symbol, snapshot in snapshots.items():
                    try:
                        ...
                        continue  # nosec B112 - skip malformed option data
                ...
                if len(quotes) > 0 and earliest_exp_in_page and earliest_exp_in_page > max_exp:
                    logger.debug(
                        f"Stopping pagination - past target date range (page earliest: {earliest_exp_in_page})"
                    )
                    break
    ...
    def place_option_order(
        self, contract_id: str, side: str, quantity: int, limit_price: Decimal | None = None
    ) -> dict:
