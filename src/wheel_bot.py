...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('wheel_bot.log'),
...
class WheelBot:
    ...
    def _load_config(self, config_path: str) -> AlpacaConfig:
        ...
        watchlist = [
            StockConfig(**{
                **stock_config,
                'max_position_size': Decimal(str(stock_config['max_position_size']))
            })
    ...
    def _is_trading_time(self, current_time: datetime) -> bool:
        ...
        if (self.config.polling.check_premarket and 
            time(4, 0) <= current_time_et < time(9, 30)):
            logger.debug(f"{current_time_et} is within pre-market hours")
            return True
        
        # After-hours (4:00 PM to 8:00 PM ET)
        if (self.config.polling.check_afterhours and 
            time(16, 0) < current_time_et < time(20, 0)):  # Note: strict inequality for market close
    ...
    def _check_positions(self) -> bool:
        ...
        for position in self.account_manager.positions:
            ...
            if position.expiration:  # Options position
                logger.debug(f"Checking option position: {position.symbol} "
                ...
                logger.info(f"Position check for {position.symbol}: DTE={days_to_expiry}, "
                ...
                logger.info(f"Looking for option with exp={exp_str}, strike={position.strike_price}, type={option_type}")
                
                for opt in current_chain:
                    opt_id = opt['id']
                    # Match by strike, type, AND expiration date
                    # OCC format: SYMBOL + YYMMDD + C/P + STRIKE (8 digits)
                    if (abs(Decimal(str(opt['strike'])) - position.strike_price) < Decimal('0.01') and 
                        option_type in opt_id and
                        exp_str in opt_id):
                        current_option = opt
                ...
                if not current_option:
                    logger.warning(f"Could not find current option quote for {position.symbol} "
                ...
                # ============================================================
                should_take_profit, profit_reason = self.strategy.should_take_profit(
                    position, current_ask
                )
                ...
                if should_take_profit:
                    ...
                    if not can_trade:
                        ...
                        # Buy to close
                        self.alpaca.place_option_order(
                            current_option['id'], "buy", abs(position.contracts)
                        )
                ...
                if days_to_expiry <= 5:
                    ...
                    if is_put and current_price > position.strike_price:
                        ...
                        logger.info(f"{position.symbol} put is ITM (${current_price:.2f} <= ${position.strike_price}), "
                                  f"allowing assignment per wheel strategy")
                    elif not is_put and current_price >= position.strike_price:
                        # ITM call - let shares get called away
                        logger.info(f"{position.symbol} call is ITM (${current_price:.2f} >= ${position.strike_price}), "
                                  f"allowing assignment per wheel strategy")
                else:
                ...
                if should_roll and option_chain:
                    ...
                    if is_put:
                        new_option = self.strategy.analyze_put_opportunity(
                            position.symbol, price_data, option_chain
                        )
                    else:
                    ...
                    if new_option:
                        ...
                        # Open new position
                        self.alpaca.place_option_order(
                            new_option['id'], "sell", abs(position.contracts)
                        )
                        logger.info(f"Rolled {position.symbol} from strike {position.strike_price} "
            ...
            # Handle Stocks (Selling Covered Calls)
                ...
                has_cc = any(
                    ...
                    p.position_type == PositionType.COVERED_CALL
                ...
                call = self.strategy.analyze_call_opportunity(
                    position.symbol,
                ...
                if call:
                    ...
                    logger.info(f"Selling {num_contracts} CC for {position.symbol} Strike={call['strike']} Premium={call['bid']}")
    ...
    def _look_for_entries(self):
        ...
        for stock in self.config.watchlist:
            ...
            if existing_position:
                logger.info(f"Already have {existing_position.position_type.value} position in {stock.symbol}, skipping entry")
                continue
